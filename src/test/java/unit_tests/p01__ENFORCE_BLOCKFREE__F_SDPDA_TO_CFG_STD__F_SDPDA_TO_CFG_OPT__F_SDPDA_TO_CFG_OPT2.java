 ...
 */
public class p01__ENFORCE_BLOCKFREE__F_SDPDA_TO_CFG_STD__F_SDPDA_TO_CFG_OPT__F_SDPDA_TO_CFG_OPT2 {
    ...
    public void Test() {
        ...
        try {
            ...
            //
            cfg resultOPT2 = ((option__Some<cfg>) T09_SDPDA_TO_CFG_OPT2.F_SDPDA_TO_CFG_OPT2(F_SDPDAtoCFG_example_SDPDA_empty_stack_marking, null)).the();
            System.out.println();
            new cfgLM_semantics<>().print_accessible_configurations(resultOPT2, 15, true);

            for (Object elem : resultOPT2.get_cfg_nonterminals())
                if (!cfg_TRIM.get_cfg_nonterminals().contains(elem))
                    System.out.println("N: resultOPT2 - cfg_TRIM : " + elem);

            for (Object elem : cfg_TRIM.get_cfg_nonterminals())
                if (!resultOPT2.get_cfg_nonterminals().contains(elem))
                    System.out.println("N: cfg_TRIM - resultOPT2: " + elem);


            for (Object elem : resultOPT2.get_cfg_step_labels())
                if (!cfg_TRIM.get_cfg_step_labels().contains(elem))
                    System.out.println("E: resultOPT2 - cfg_TRIM : " + elem);

            for (Object elem : cfg_TRIM.get_cfg_step_labels())
                if (!resultOPT2.get_cfg_step_labels().contains(elem))
                    System.out.println("E: cfg_TRIM - resultOPT2: " + elem);


            cfg cfgOPT2_TRIM = ((option__Some<cfg>) T03_06_05_CFG_TRIM.F_CFG_TRIM(resultOPT2)).the();
            System.out.println();
            new cfgLM_semantics<>().print_accessible_configurations(cfgOPT2_TRIM, 15, false);
            System.out.println("done cfgOPT2_TRIM");
...
//
            ...
            System.out.println("resultOPT2.get_cfg_step_labels().size() = " + resultOPT2.get_cfg_step_labels().size());
            System.out.println("resultOPT2.get_cfg_nonterminals().size() = " + resultOPT2.get_cfg_nonterminals().size());
            System.out.println("cfgOPT2_TRIM.get_cfg_step_labels().size() = " + cfgOPT2_TRIM.get_cfg_step_labels().size());
            System.out.println("cfgOPT2_TRIM.get_cfg_nonterminals().size() = " + cfgOPT2_TRIM.get_cfg_nonterminals().size());

 ...
 */
public class p01__ENFORCE_BLOCKFREE__T05_04_LR_MACHINE___FUNLRMACHINE_example3 {
    ...
    public void Test() {
        ...
        try {
            ...
            epda<General_Set<cfg_item<symbol__abstract<string_symbol>, symbol__abstract<string_symbol>>>, two_elements__abstract<symbol__abstract<string_symbol>, symbol__abstract<string_symbol>>, General_Integer>
                    output = T05_04_LR_MACHINE.F_LR_MACHINE(input_augmented, 1);

            assert (output.get_epda_marking().isEmpty());
            assert (output.get_epda_delta().size() == 44);
            assert (output.get_epda_states().size() == 30);
            assert (output.get_epda_gamma().size() == 1);
            assert (output.get_epda_events().size() == 10);
